...
function SidebarGeometry(editor) {
  ...
  geometryTypeRow.add(
    new UIText(strings.getKey("sidebar/geometry/type")).setWidth("90px")
  );
  ...
  var geometryUUID = new UIInput()
    .setWidth("102px")
    .setFontSize("12px")
    .setDisabled(true);
  var geometryUUIDRenew = new UIButton(strings.getKey("sidebar/geometry/new"))
    .setMarginLeft("7px")
    .onClick(function () {
  ...
  geometryUUIDRow.add(
    new UIText(strings.getKey("sidebar/geometry/uuid")).setWidth("90px")
  );
  ...
  geometryNameRow.add(
    new UIText(strings.getKey("sidebar/geometry/name")).setWidth("90px")
  );
  ...
  container.add(
    new UIText(strings.getKey("sidebar/geometry/bounds")).setWidth("90px")
  );
  ...
  var vertexNormalsButton = new UIButton(
    ...
    if (object && object.geometry) {
      ...
      if (currentGeometryType !== geometry.type) {
        ...
        if (geometry.type === "BufferGeometry") {
          ...
          var { GeometryParametersPanel } = await import(
      ...
      geometryBoundingSphere.setValue(
        Math.floor(geometry.boundingSphere.radius * 1000) / 1000
      );
    } else {
      container.setDisplay("none");
    }
